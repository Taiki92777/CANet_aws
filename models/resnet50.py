...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, norm_layer=None,
        ...
        if not self.crossCBAM:
            self.layer5 = nn.Conv2d(
                2048, 300, kernel_size=3, padding=1, bias=False)
        ...
        if self.liu:
            ...
            self.classifier1 = nn.Linear(
                512 * block.expansion+256, self.num_classes)
            ...
            self.branch_bam1 = CBAM(512 * block.expansion)
            self.branch_bam2 = CBAM(512 * block.expansion)
            self.classifier_specific_1 = nn.Linear(
                512 * block.expansion, self.num_classes)
    ...
    def forward(self, x):
        ...
        if self.chen:
            ...
            out1 = self.classifier1_3(self.dropout(
                self.classifier1_2(self.dropout(self.classifier1_1(out)))))
            out2 = self.classifier2_3(self.dropout(
                self.classifier2_2(self.dropout(self.classifier2_1(out)))))
            return out1, out2
        if self.liu:
            out = self.dropout(out)
...
def image_cv(x1):
    import cv2
    filename = ["IDRiD_018", 'IDRiD_031', 'IDRiD_076',
                'IDRiD_002', 'IDRiD_009', 'IDRiD_021']
    for num in range(len(filename)):
        for i in range(0, 30):
            raw_img = cv2.imread("visual_result/" +
                                 filename[num] + "/" + filename[num] + ".jpg")
